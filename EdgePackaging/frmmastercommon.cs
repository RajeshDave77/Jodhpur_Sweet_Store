...
{
    public partial class frmmastercommon : Form
    {
        private MainIndex mainForm = null;
        public frmmastercommon(Form callingForm)
        {
            mainForm = callingForm as MainIndex;
            InitializeComponent();
        }
        GetSet GS = new GetSet();
        private void frmmastercommon_Load(object sender, EventArgs e)
        {
            //pnlCommonDetails.Hide();
          
                pnlBasicInformation.Text = "Inventory Group Master";
                pnlCommonDetails.Text = "Inventory Group Details";
        ...
        private void dgvCommonMasterDetails_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            else if (e.ColumnIndex == dgvCommonMasterDetails.Columns["Delete"].Index && e.RowIndex >= 0)
            {
                try
                {
                    ...
                    int a = AllCommonFunctions.Check_Used("InventoryGroup_Master", id);
                    if(a>0)
                    {
                        MessageBox.Show("This Group Is In Use. Cannot Be Deleted.", "Edge Solutions", MessageBoxButtons.OKCancel, MessageBoxIcon.Asterisk);
                        return;
                    }
        ...
        private void frmmastercommon_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        {
            DataSet ds = new DataSet();
            ds = AllCommonFunctions.Common_SearchData("group",txtNameSearch.Text);
            dgvCommonMasterDetails.DataSource = ds.Tables[0];
            dgvCommonMasterDetails.Columns["ID"].Visible = false;
            foreach (DataGridViewRow row in dgvCommonMasterDetails.Rows)
            {
