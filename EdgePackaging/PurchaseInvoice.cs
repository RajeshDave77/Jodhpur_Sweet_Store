...
namespace EdgePackaging
{
    public partial class PurchaseInvoice : Form
    {
        ...
        public PurchaseInvoice(Form callingForm)
        {
    ...
    //        cmbparty.Text = "";
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
          ...
          //              }
                ...
                //        {
                       ...
                       int mastid = pic.Stock_InwardMaster(id, partyname, Convert.ToInt32(cmbparty.SelectedValue), cmbStockType.Text, txtinvoice.Text, dtinvoice.Text, txtremark.Text, Convert.ToInt32(cmbCompany.SelectedValue), Convert.ToInt32(Form1.UserID), Convert.ToInt32(txtEntry.Text), Form1.CurrFinYear,0);
            ...
            //        }
                  ...
                  else if (btnSave.Text == "&Update")
                  {
                      ...
                      int mastid = pic.Stock_InwardMaster(upid, partyname, Convert.ToInt32(cmbparty.SelectedValue), cmbStockType.Text, txtinvoice.Text, dtinvoice.Text, txtremark.Text, Convert.ToInt32(cmbCompany.SelectedValue), Convert.ToInt32(Form1.UserID), Convert.ToInt32(txtEntry.Text), Form1.CurrFinYear,0);
         ...
         //   da.fill_combo("select id,(name) as name from ledger_Master", cmbCompany, "ID", "Name");
          //  cmbparty.SelectedValue = Form1.CompanyID;
            ...
            da.fill_combo(" select 0 as ID, '--Select--' as Name,1 as rtag union all select distinct ID, Name,2 as rtag from tblunit order by rtag,Name", cmbUnit, "ID", "Name");
                ...
                {
                    da.fill_combo("select 0 as id,'---Select---' as name,1 as rtag union all select id,(name) as name,2 as rtag from ledger_Master where type in ('Department','Shop') and companyid=" + cmbCompany.SelectedValue + " order by  rtag,name", cmbSubLocation, "ID", "Name");
        ...
        private void ShowBatchData(int productid)
        {
            ...
            rsTemp = da.dataset_ret("select a.masterid,a.id,a.code,a.name,a.subgroupname,a.exdate,(((isnull(aaa.qty,0)+isnull(inward.inward,0))-(isnull(outward.outward,0)+isnull(consumed.consumed,0)+isnull(damages.damages,0)+isnull(wastage.wastage,0)))) as stock " +
                                          "  ,a.unit from ( " +
                                          "  (select stockinwarddetail.masterid, inventory_master.id,code,name,batchno as subgroupname,inventory_master.unit,exdate from Inventory_Master inner join stockinwarddetail on stockinwarddetail.product=inventory_master.id inner join stockinwardmaster on stockinwardmaster.id=masterid where  stockinwardmaster.stocktype='Purchase' and product="+productid+") as a left join  " +
                                          "  (select aa.product,sum(aa.qty) as qty from( select id as  product,opening_stock as qty from inventory_master ) as aa group by aa.product) as aaa on a.id=aaa.product " +
